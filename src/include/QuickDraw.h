...
using QDCommentUPP = UPP<void(INTEGER kind, INTEGER size, Handle data)>;
using QDTexMeasUPP = UPP<INTEGER(INTEGER bc, Ptr texta, GUEST<Point> *numer, GUEST<Point> *denom, FontInfo *info)>;
using QDGetPicUPP = UPP<void(void * data, INTEGER bc)>;
using QDPutPicUPP = UPP<void(const void * data, INTEGER bc)>;

struct QDProcs
{
    GUEST_STRUCT;
    GUEST<QDTextUPP> textProc;
    GUEST<QDLineUPP> lineProc;
    GUEST<QDRectUPP> rectProc;
    GUEST<QDRRectUPP> rRectProc;
    GUEST<QDOvalUPP> ovalProc;
    GUEST<QDArcUPP> arcProc;
    GUEST<QDPolyUPP> polyProc;
    GUEST<QDRgnUPP> rgnProc;
    GUEST<QDBitsUPP> bitsProc;
    GUEST<QDCommentUPP> commentProc;
    GUEST<QDTexMeasUPP> txMeasProc;
    GUEST<QDGetPicUPP> getPicProc;
    GUEST<QDPutPicUPP> putPicProc;
};
...
extern void C_DrawString(ConstStringPtr s);
...
extern INTEGER C_StringWidth(ConstStringPtr s);
