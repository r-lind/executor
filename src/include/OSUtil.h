...
extern ProcPtr C_NGetTrapAddress(uint16_t n, TrapType ttype);
NOTRAP_FUNCTION(NGetTrapAddress);
extern void C_NSetTrapAddress(ProcPtr addr, uint16_t n, TrapType ttype);
NOTRAP_FUNCTION(NSetTrapAddress);

extern ProcPtr _GetTrapAddress_flags(uint16_t n, bool newTraps, bool tool);
REGISTER_2FLAG_TRAP(_GetTrapAddress_flags, 
    GetTrapAddress, INVALID_GetTrapAddress, GetOSTrapAddress, GetToolTrapAddress, 
    0xA146, ProcPtr(uint16_t), A0(D0, TrapBit<0x200>, TrapBit<0x400>), ClearD0);

extern void _SetTrapAddress_flags(ProcPtr addr, uint16_t n, bool newTraps, bool tool);
REGISTER_2FLAG_TRAP(_SetTrapAddress_flags, 
    SetTrapAddress, INVALID_SetTrapAddress, SetOSTrapAddress, SetToolTrapAddress, 
    0xA047, void(ProcPtr,uint16_t), void(A0, D0, TrapBit<0x200>, TrapBit<0x400>), ClearD0);

extern ProcPtr C_GetToolboxTrapAddress(uint16_t n);
NOTRAP_FUNCTION(GetToolboxTrapAddress);
extern void C_SetToolboxTrapAddress(ProcPtr addr, uint16_t n);
