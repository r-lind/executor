...
int16_t Executor::te_char_to_lineno(TEPtr te, int16_t sel)
{
    ...
    while(low < high
          && (current_elt = line_starts[current]) != sel)
    {
        if(current_elt < sel)
            low = current + 1;
        else
...
int16_t calclhtab(TEHandle teh)
{
        ...
        linestarts <= (TE_LINE_STARTS(teh) + n_lines);)
    {
        ...
        if(STYLE_RUN_START_CHAR(current_run + 1) > linestarts[1])
        {
            if(first_changed == -1
        ...
        else if(STYLE_RUN_START_CHAR(current_run + 1) < linestarts[1])
            current_run++;
        else
        {
            if(first_changed == -1
...
int16_t Executor::ROMlib_call_TEDoText(TEPtr tp, int16_t first, int16_t last, int16_t what)
{
    ...
    if(LM(TEDoText) == (ProcPtr)&ROMlib_dotext)
